...
if st.button("🚀 BAŞLAT", type="primary", use_container_width=True):
    ...
    for idx, cat in enumerate(selected_categories):
        status.info(f"🔍 {cat} taranıyor (Apify)...")
        try:
            # Scraper çağrısı
            ...
            if isinstance(result, dict) and result.get("error"):
                status.error(f"❌ {cat} taranamadı!")
                if result.get("run_url"):
                    st.link_button(f"🔍 Hatayı Apify'da Gör ({cat})", result["run_url"])
                    st.warning("Yukarıdaki linke tıkla. Eğer 'Blocked' yazıyorsa Apify engellenmiştir.")
                products = []
            else:
                products = result
            ...
            if products:
                ...
                status.success(f"✅ {cat}: {scraped_count} ürün çekildi.")

        except Exception as e:
            status.error(f"❌ {cat} kod hatası: {e}")
    ...
    if all_results:
        ...
        st.error("❌ Hiçbir ürün çekilemedi.")
